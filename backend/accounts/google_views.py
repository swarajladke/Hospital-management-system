    ...
    def get(self, request):
        ...
        try:
            ...
            if credentials.refresh_token:
                ...
                return redirect(f'{frontend_url}/settings?success=google_connected')
            else:
                ...
                if not user_profile.google_refresh_token:
                     return redirect(f'{frontend_url}/settings?error=no_refresh_token')
                return redirect(f'{frontend_url}/settings?success=google_reconnected')
                 
        except Exception as e:
            logger.error(f"Error in Google Callback: {e}")
            return redirect(f'{frontend_url}/settings?error=exception')
